...
def compute_plucker_coordinates(extrinsic_matrix, intrinsic_matrix, H, W):
    ...
    :param extrinsic_matrix: [4, 4] tensor representing the camera extrinsic matrix
    ...
    # Stack the coordinates and transform to camera space
    ndc = torch.stack([ndc_x, ndc_y, ndc_z], dim=-1).reshape(-1, 3)

    # Apply the extrinsic matrix to transform points to camera coordinates
    extrinsic_inv = torch.inverse(extrinsic_matrix)
    rays_camera = torch.cat([ndc, torch.ones(ndc.shape[0], 1).to(device)], dim=-1) @ extrinsic_inv.T
    ray_origins = rays_camera[:, :3]

    # Normalize ray directions
    ray_directions = ndc / ndc.norm(dim=-1, keepdim=True)

    # Compute the Plücker normal
    plucker_normal = torch.cross(ray_origins, ray_directions, dim=-1)

    # Combine directions and Plücker normal into a new ray tensor
