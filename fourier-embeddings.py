...
def fourier_features(x, L):
    ...
    Apply cos to the x or i index, and sin to the y or j index.
    """
    cos_features = torch.cos((2**L) * x[:, :, 0:1])  # Apply cos to x or i index
    sin_features = torch.sin((2**L) * x[:, :, 1:2])  # Apply sin to y or j index
    fourier_feats = torch.cat([cos_features, sin_features], dim=-1)  
...
def compute_fourier_feature_map(B, H, W, scales):
    ...
    coord_map = coord_map.reshape(B, -1, 2)  # Shape: (B, H*W, 2)
    # print("$$$$$$$$$$$$$$")
...
##################ray map includes direction embedding as well, please uncomment############### 
##################this and comment the same name function above################################
...
#############ray map includign pixel location and direction embedding############
# B, H, W = 8, 32, 32
# scales = [1, 4, 7]  # Scales for Fourier features
